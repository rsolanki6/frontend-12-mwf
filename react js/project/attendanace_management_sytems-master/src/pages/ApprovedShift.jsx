...
export default function ApprovedShift() {
  const [items, setItems] = useState([]);
  useEffect(() => setItems(getShifts().filter(s => s.status === "approved")), []);

  return <ShiftList title="Approved Shift" subtitle="All approved employee shift requests." items={items} />;
}

function ShiftList({ title, subtitle, items }) {
  return (
    <div className="mx-auto max-w-7xl">
      <div className="mb-6"><h2 className="text-2xl font-bold text-slate-800">{title}</h2><p className="mt-1 text-sm text-slate-400">{subtitle}</p></div>
      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        {items.length === 0 ? <EmptyState title="No approved shifts" text="Approved shift requests will appear here." /> : (
